...
function sh:constructDefinesGlobal(dream)
	return [[
		//todo prototype, cleanup required
		float sampleShadowSun22(Image tex, vec2 shadowUV, float depth, float distanceFactor, bool dynamic) {
			if (dynamic) {
				vec2 r = texture(tex, shadowUV).xy;
				return clamp(exp(4.0 * (min(r.x, r.y) * 40.0 * distanceFactor - depth * 40.0)), 0.0, 1.0);
			} else {
				float r = texture(tex, shadowUV).x;
				return clamp(exp(4.0 * (r * 40.0 * distanceFactor - depth * 40.0)), 0.0, 1.0);
			}
		}
		
		float sampleShadowSun2(Image tex, vec2 shadowUV, float depth, float distanceFactor, bool dynamic) {
			...
			return (
				...
				sampleShadowSun22(tex, shadowUV + vec2(-1.5 + ox, -1.5 + oy) * ss_texelSize, depth, distanceFactor, dynamic) +
				sampleShadowSun22(tex, shadowUV + vec2(0.5 + ox, -1.5 + oy) * ss_texelSize, depth, distanceFactor, dynamic)
			) * 0.25;
		}
