...
function sh:constructDefinesGlobal(dream)
	...
	vec3 sampleOffsetDirectionsDynamic[20] = vec3[] (
	...
	float sampleShadowPointSmoothDynamic(vec3 lightVec, samplerCube tex) {
		...
		for (int i = 0; i < 20; ++i) {
			vec2 f = texture(tex, n + sampleOffsetDirectionsDynamic[i] * diskRadius).xy;
...
function sh:constructDefines(dream, ID)
	return ([[
		...
		extern bool point_shadow_dynamic_smooth_#ID#;
...
function sh:constructPixel(dream, ID)
	return ([[
		...
		float shadow;
		if (point_shadow_dynamic_smooth_#ID#) {
			shadow = sampleShadowPointSmoothDynamic(lightVec, point_shadow_dynamic_tex_#ID#);
		} else {
			shadow = sampleShadowPointDynamic(lightVec, point_shadow_dynamic_tex_#ID#);
		}
...
function sh:sendUniforms(dream, shaderObject, light, ID)
	...
	if light.shadow.canvas then
		if light.smooth == nil then
			shader:send("point_shadow_dynamic_smooth_" .. ID, dream.shadow_smooth)
		else
			shader:send("point_shadow_dynamic_smooth_" .. ID, light.smooth)
		end
