...
function job:execute(times, delta, obj, pos, face)
	...
	local depth_buffer = love.graphics.newCanvas(canvas:getWidth(), canvas:getHeight(), {format = "depth16", readable = false, msaa = canvas:getMSAA()})
	love.graphics.setCanvas({{canvas, face = face}, depthstencil = depth_buffer})
	love.graphics.clear()
	obj.reflection.canvas = nil
	
	--render
	lib:renderFull(cam, lib.canvases_reflections, true)
