...
for bounce = 1, 5 do
	local origin = vec3(1, 2, 3)
	local direction = vec3(1, 2, 3):normalize()
	local result = raytrace:cast(tavern, origin, direction)
	if result then
		if bounce > 1 then
			local dir = result:getPosition() - origin
			local particles = dir:length() * 30
			for i = 1, particles do
				local p = (origin + dir * (i - 0.5) / particles - vec3(centerX, centerY, centerZ)) * resolution
		...
		origin = result:getPosition() - direction:normalize() * 0.01
		local n = vec3(love.math.noise(bounce, 1) - 0.5, love.math.noise(bounce, 2) - 0.5, love.math.noise(bounce, 3) - 0.5)
		direction = (result:getNormal() + n * 0.1) * 100
	else
		break
	end
end
